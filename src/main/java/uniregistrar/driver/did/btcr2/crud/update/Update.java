...
public class Update {
    ...
    public static UpdateProcessUpdateSignPayloadResult announceToAggregateBeaconCAS(BitcoinConnection bitcoinConnection, AggregationCohort aggregationCohort, int participantIndex, DID did, DIDDocument didSourceDocument, BTCR2Update btcr2Update, URI verificationMethodId) throws RegistrationException {
        ...
        aggregationCohort.setCasUpdateHash(participantIndex, ByteBuffer.wrap(updateHash));
        ...
        PublicKey participantPublicKey = new PublicKey(aggregationCohort.getParticipantPublicKeys().get(participantIndex).array());
        List<PublicKey> participantPublicKeys = aggregationCohort.getParticipantPublicKeys().stream().map(ByteBuffer::array).map(PublicKey::new).toList();
        ...
        aggregationCohort.setMusig2SecretNonce(participantIndex, ByteBuffer.wrap(pair.component1().getData$bitcoin_kmp().getBytes$bitcoin_kmp()));
        aggregationCohort.setMusig2IndividualNonce(participantIndex, ByteBuffer.wrap(pair.component2().toByteArray()));
    ...
    public static UpdateProcessUpdateSignPayloadResult announceToAggregateBeaconSMT(BitcoinConnection bitcoinConnection, AggregationCohort aggregationCohort, int participantIndex, DID did, DIDDocument didSourceDocument, BTCR2Update btcr2Update, URI verificationMethodId, byte[] nonce) throws RegistrationException {
        ...
        aggregationCohort.setSmtDidIndex(participantIndex, ByteBuffer.wrap(didIndex));
        ...
        aggregationCohort.setSmtUpdateHash(participantIndex, ByteBuffer.wrap(updateHash));
        aggregationCohort.setSmtNonce(participantIndex, nonce != null ? ByteBuffer.wrap(nonce) : ByteBuffer.wrap(new byte[0]));
